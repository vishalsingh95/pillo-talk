...
if(isset($_GET['ord_sts']))
{
  $ordre_status=$_GET['ord_sts'];
}
...
<li class="breadcrumb-item active" style="font-size: 18px;font-weight: bold;color: black !important;"><?php if(isset($_GET['ord_sts']))
{ ?><?php if($ordre_status=="Ordered"){ ?> Pending <?php  }else { echo $ordre_status;?> <?php } } ?> Orders</li>
</ol>
<?php 
if(isset($_GET['ord_sts']))
{
...
//my code start
$get_pnding_ordr="SELECT DISTINCT `address_of_orders`. `order_id` FROM `address_of_orders` LEFT JOIN `orders` ON `orders`.`order_id` = `address_of_orders`.`order_id` WHERE `orders`.`status` = '$ordre_status'";
$qst_get_pnding_ordr=$db->query($get_pnding_ordr);
while($clct_get_pnding_ordr=$qst_get_pnding_ordr->fetch_assoc())
{
   $pending_ords_id=$clct_get_pnding_ordr['order_id'];
    
    $ord_dtl="select * from `address_of_orders` where `order_id`='$pending_ords_id' order by `id` DESC";
$qst_ord_dtl=$db->query($ord_dtl);
    $clct_ord_dtl=$qst_ord_dtl->fetch_assoc();
...
<tbody>
<?php 
$clt_ord_dtl="select * from `orders` where `order_id`='$ord_id' and `status`='$ordre_status'";
...
$reason_c=$clct_clt_ord_dtl['cancel_reason'];
...
"></td>
        ...
        <td class="text-success"><b><?php echo $prdt_stts;?> <?php if($ordre_status=="Canceled") { ?><span style="color:red">( <?php echo $reason_c; ?> )</span><?php } ?></b></td>
...
<tr><td colspan="7"></td><td><a href="view_order_details.php?ord_id=<?php echo $ord_id; ?>" class="btn btn-warning" style="
    /* display: flex; */
">View Full Order details</a></td></tr>
</tbody>
