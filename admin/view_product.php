...
<div class="col-md-12 mb-5" ><a href="add_product.php" style='float:right' class='btn btn-primary'>Add product</a></div>
...
<th style="border-bottom:2px solid black !important;">Product type</th>
...
$sno=1;
    ...
    {
        $product_name=$clct_usr_lst['product_name'];
        $prdt_image=$clct_usr_lst['product_image'];
        $price=$clct_usr_lst['price'];
        $prdt_type=$clct_usr_lst['product_type'];
        $prdit_id=$clct_usr_lst['id'];
        
           $ctgy_id=$clct_usr_lst['category_id'];
       $clt_ctgy_dtl="select * from `product_category` where `id`='$ctgy_id'";
       $qst_clt_ctgy_dtl=$db->query($clt_ctgy_dtl);
       
       $clct_fetch_ctgy=$qst_clt_ctgy_dtl->fetch_assoc();
       
       $ctgy_neme=$clct_fetch_ctgy['name'];
       
     $prdt_imge=explode(", ",$prdt_image);
    ...
    ?><tr>
        ...
        <td><img src="product_images/<?php echo $prdt_imge[0];?>" style="height:50px; width:50px;"></td>
       
        <td><?php echo $prdt_type;?></td>
        <td><?php echo $ctgy_neme;?></td>
        <td><a  href="edit_product.php?id=<?php echo $prdit_id;?>" class='btn btn-success'><i class='fa fa-edit'></i></a> <a href="delete_product.php?id=<?php echo $prdit_id;?>" onclick="return confirm('Are you sure want to delete this product?')"  class='btn btn-danger'><i class='fa fa-trash'></i></a></td>
        </tr>
        <?php
    }
    ?>
